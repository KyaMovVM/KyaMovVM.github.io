...
Поисковым системам следует использовать домен https://kyamovvm.com. Сайт разворачивается в каталоге `/static`, поэтому в `robots.txt` указана ссылка на `https://kyamovvm.com/static/sitemap.xml`, где перечислены все HTML‑страницы. Директива `Host: kyamovvm.com` предотвращает индексацию github.io.
