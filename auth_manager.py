...
def create_account(username: str, password: str = '', ssh_key: str = '', enable_mfa: bool = False) -> Account:
    accounts = load_accounts()
    secret = pyotp.random_base32() if enable_mfa else ''
    acc = Account(username=username, password=password, ssh_key=ssh_key, mfa_secret=secret)
...
def authenticate(username: str, password: str = '', ssh_key: str = '', mfa_code: str = '') -> bool:
...
if __name__ == '__main__':
    ...
    if args.cmd == 'create':
        acc = create_account(args.username, args.password or '', args.ssh_key or '', args.mfa)
        print('Аккаунт создан')
        if acc.mfa_secret:
            print('MFA secret:', acc.mfa_secret)
            print('Текущий код:', pyotp.TOTP(acc.mfa_secret).now())
    elif args.cmd == 'auth':
        ok = authenticate(args.username, args.password or '', args.ssh_key or '', args.mfa_code)
